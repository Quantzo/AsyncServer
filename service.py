import abc
import uuid
from asyncio import StreamReader, StreamWriter, ensure_future, AbstractEventLoop
...
class Service(metaclass=abc.ABCMeta):
    ...
    async def __listen_to_client(self, clientid: uuid.UUID, reader: StreamReader)->None:
        if reader != None:
            try:
                ...
                await self.client_disconnected(clientid)
    ...
    async def client_connected(self, reader: StreamReader, writer: StreamWriter):
        if reader != None:
            ...
            await self.on_client_connected(reader, writer, clientid)
            ensure_future(self.__listen_to_client(clientid, reader), loop=self._loop)
    ...
    async def client_disconnected(self, clientid: uuid.UUID):
        self.__client_list = list(filter(lambda i: i[0] != clientid, self.__client_list))
        print("Client disconected: "+ str(clientid))
        await self.broadcast_text_message("Client disconected: "+ str(clientid))
